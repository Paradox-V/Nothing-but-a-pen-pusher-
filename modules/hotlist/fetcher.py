...
class DataFetcher:
    ...
    def fetch_all_platforms(
        ...
        """Fetch hot items from all (or selected) platforms in one call.

        Args:
            platform_ids: optional list of platform IDs to fetch.  When
                *None*, every key in :data:`PLATFORM_NAMES` is used.

        Returns:
            A two-element tuple ``(items, failed_ids)``.

            * **items** -- a flat list of dicts, each with keys
              ``title``, ``url``, ``platform``, ``platform_name``,
              ``hot_rank``, ``hot_score``.
            * **failed_ids** -- platform IDs that could not be fetched.
        """
        ...
        results, _id_to_name, failed_ids = self.crawl_websites(ids_list)

        items: List[Dict] = []
        for platform_id, title_map in results.items():
            for title, info in title_map.items():
                rank = min(info["ranks"]) if info["ranks"] else None
                items.append(
                    {
                        "title": title,
                        "url": info.get("url", ""),
